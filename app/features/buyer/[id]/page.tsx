...
export default function BuyerProfilePage({
  ...
  const [savedProduceDetails, setSavedProduceDetails] = useState<SavedProduceDetails[]>([]);
  ...
  const loadSavedProduceFromLocalStorage = async () => {
    try {
      ...
      if (ids.length > 0) {
        ...
        setSavedProduceDetails(produceDetails.filter((p) => p !== null) as SavedProduceDetails[]);
      } else {
    ...
    <ProtectedRoute>
      <div className="bg-[#f5f0e8] min-h-screen relative">
        ...
        <div className="fixed inset-0 pointer-events-none overflow-hidden">
          <div className="absolute -right-20 top-1/4 text-[15rem] font-black text-[#1a3d2b]/3 select-none whitespace-nowrap" style={{ transform: "rotate(90deg)", transformOrigin: "center" }}>
            NERTHUS
          </div>
          <div className="absolute -left-40 bottom-1/4 text-[20rem] font-black text-[#e86c2a]/2 select-none whitespace-nowrap" style={{ transform: "rotate(-90deg)", transformOrigin: "center" }}>
            ORGANIC
          </div>
        ...
        <div className="bg-linear-to-br from-[#1a3d2b] via-[#1a3d2b] to-[#0e2a1d] py-20 px-6 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto relative">
            <div className="flex justify-between items-start mb-8">
              <Link href="/features/buyer" className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-white/40 hover:text-white transition-colors">
                <ArrowLeft size={13} /> Back to Marketplace
              </Link>
              {isOwner && (
                <button onClick={handleLogout} disabled={loggingOut} className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-white/40 hover:text-white border border-white/20 hover:border-white/40 px-4 py-1.5 rounded-xl transition-colors disabled:opacity-50">
            ...
            <div className="flex items-start gap-6">
              ...
              <div>
                <div className="flex items-center gap-3 mb-2">
                  <span className="text-[10px] font-black uppercase tracking-[0.3em] text-[#e8c84a] bg-[#e8c84a]/10 px-3 py-1 rounded-full">Verified Buyer</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1">
              <div className="space-y-4 mb-8">
                ...
                {!isOwner && (
                  <button onClick={toggle} className={`w-full flex items-center justify-center gap-2 text-[11px] font-black uppercase tracking-widest px-5 py-3 rounded-xl transition-all ${isFollowing ? "bg-[#f5f0e8] text-[#1a3d2b] border-2 border-[#1a3d2b]/20 hover:border-[#1a3d2b]/40" : "bg-[#1a3d2b] text-[#e8c84a] hover:bg-[#2a5a3b] shadow-lg"}`}>
            ...
            <div className="lg:col-span-2">
              <div className="flex gap-1 mb-8 bg-white rounded-xl p-1.5 border border-[#d4c9b0] w-fit">
                {[
                  ...
                  <button key={label} onClick={() => setActiveTab(i)} className={`flex items-center gap-2 px-5 py-2.5 text-sm font-bold uppercase tracking-wider rounded-lg transition-all ${activeTab === i ? "bg-[#1a3d2b] text-[#e8c84a] shadow-sm" : "text-[#4a5a4e] hover:text-[#1a3d2b] hover:bg-[#f5f0e8]"}`}>
                    <Icon size={14} />
                    {label}
                  </button>
                ))}
                        ...
                        <div key={order._id} className="bg-white border border-[#d4c9b0] rounded-xl p-5 hover:border-[#1a3d2b]/40 hover:shadow-md transition-all">
                          <div className="flex flex-wrap justify-between items-start mb-4">
                            <div>
                              ...
                              <p className="text-xs text-[#8a9a8e] mt-1">{new Date(order.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })}</p>
                            </div>
                            <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${order.status === "delivered" ? "bg-green-100 text-green-700" : order.status === "cancelled" ? "bg-red-100 text-red-700" : "bg-yellow-100 text-yellow-700"}`}>
                              {order.status}
                            </span>
                          </div>
                          <div className="border-t border-[#d4c9b0] pt-4 mb-4">
                            {order.items.slice(0, 2).map((item, idx) => (
                              <div key={idx} className="flex items-center gap-3 mb-2">
                                <div className="w-10 h-10 rounded-lg overflow-hidden bg-[#f5f0e8] relative">
                                  {item.img ? <Image src={item.img} alt={item.name} fill className="object-cover" /> : <Package size={16} className="absolute inset-0 m-auto text-[#8a9a8e]" />}
                                </div>
                            ...
                            {order.items.length > 2 && <p className="text-xs text-[#8a9a8e] mt-2">+ {order.items.length - 2} more items</p>}
                          </div>
                  ...
                  {savedProduceLoading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {savedProduceDetails.map((produce) => (
                        <Link key={produce._id} href={`/features/produce/${produce._id}`} onClick={() => dispatch(markProduceVisited(produce._id))} className="bg-white border border-[#d4c9b0] rounded-xl overflow-hidden hover:border-[#1a3d2b]/40 hover:shadow-md transition-all group">
                          <div className="relative w-full h-36 bg-[#f5f0e8] overflow-hidden">
                            {produce.img ? <Image src={produce.img} alt={produce.name} fill className="object-cover group-hover:scale-105 transition-transform duration-300" /> : <div className="w-full h-full flex items-center justify-center"><Package size={32} className="text-[#8a9a8e]" /></div>}
                            {produce.isOrganic && <span className="absolute top-2 left-2 bg-[#1a3d2b] text-[#e8c84a] text-[8px] font-black px-2 py-0.5 rounded-full">ORGANIC</span>}
                            <button className="absolute top-2 right-2 bg-white/90 rounded-full p-1.5 shadow-sm" onClick={(e) => {
                              e.preventDefault();
                              const updated = savedProduceIds.filter((id) => id !== produce._id);
                              localStorage.setItem("nerthus_savedProduce", JSON.stringify(updated));
                              loadSavedProduceFromLocalStorage();
                            }}>
                          ...
                          <div className="p-3">
                            <h4 className="font-bold text-[#1a3d2b] text-sm">{produce.name}</h4>
                            <p className="text-[10px] text-[#8a9a8e]">by {produce.farmerId.fullName}</p>
                            <div className="flex justify-between items-center mt-2">
                              <span className="text-base font-black text-[#e86c2a]">₹{produce.price}/{produce.unit}</span>
                              <span className="text-[10px] text-[#1a3d2b] font-bold">View →</span>
                            </div>
                          </div>
                        </Link>
                      ))}
                    ...
                    <div className="space-y-8">
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-6">
                          ...
                          <Link href="/features/produce" className="text-[10px] font-bold text-[#e86c2a] hover:underline">Browse All →</Link>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          {recommendedLoading ? Array(3).fill(0).map((_, i) => (
                            ...
                            <Link key={item._id} href={`/features/produce/${item._id}`} className="bg-white border border-[#d4c9b0] rounded-xl overflow-hidden hover:border-[#1a3d2b]/40 hover:shadow-md transition-all group">
                              <div className="relative w-full h-36 bg-[#f5f0e8] overflow-hidden">
                                {item.img ? <Image src={item.img} alt={item.name} fill className="object-cover group-hover:scale-105 transition-transform duration-300" /> : <div className="w-full h-full flex items-center justify-center"><Leaf size={32} className="text-[#8a9a8e]" /></div>}
                                {item.isOrganic && <span className="absolute top-2 left-2 bg-[#1a3d2b] text-[#e8c84a] text-[8px] font-black px-2 py-0.5 rounded-full">ORGANIC</span>}
                              </div>
                              <div className="p-3">
              ...
              {activeTab === 2 && (
                <div className="bg-white border border-[#d4c9b0] rounded-2xl p-8">
                  <div className="flex justify-between items-center mb-6">
                    ...
                    {visitedCount > 0 && (
                      <button onClick={clearHistory} className="flex items-center gap-1 text-[10px] text-[#8a9a8e] hover:text-red-500 transition-colors">
                  ...
                  {visitedLoading ? (
                    ...
                    <div className="space-y-4">
                      {visitedProduce.slice(0, 10).map((item, index) => (
                        <Link key={item._id} href={`/features/produce/${item._id}`} className="flex items-center gap-4 p-4 bg-[#f5f0e8] rounded-xl hover:bg-[#e8e0d0] transition-all group">
                          <div className="w-12 h-12 rounded-lg bg-white overflow-hidden relative flex-shrink-0">
                            {item.img ? <Image src={item.img} alt={item.name} fill className="object-cover" /> : <div className="w-full h-full flex items-center justify-center"><Package size={20} className="text-[#8a9a8e]" /></div>}
                          </div>
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <h3 className="font-bold text-[#1a3d2b] text-sm">{item.name}</h3>
                              {item.isOrganic && <span className="text-[8px] bg-[#1a3d2b] text-[#e8c84a] px-1.5 py-0.5 rounded-full">ORGANIC</span>}
                            </div>
                            <p className="text-[10px] text-[#8a9a8e]">by {item.farmerId?.fullName || 'Local Farmer'}</p>
                            <div className="flex items-center gap-2 mt-1">
                              <span className="text-sm font-black text-[#e86c2a]">₹{item.price}</span>
                      ...
                      {visitedCount > 10 && <p className="text-center text-[10px] text-[#8a9a8e] pt-2">+{visitedCount - 10} more items</p>}
                    </div>
                  ) : (
                    <div className="text-center py-12">
                      ...
                      <Link href="/features/produce" className="inline-flex items-center gap-2 mt-6 text-[10px] font-bold text-[#e86c2a] hover:underline">
                    ...
                    {recommendedLoading ? (
                      ...
                      <div className="space-y-3">
                        {recommended.slice(0, 3).map((item, index) => (
                          <Link key={item._id} href={`/features/produce/${item._id}`} className="flex items-center gap-4 bg-white border border-[#d4c9b0] rounded-xl p-4 hover:border-[#1a3d2b]/40 hover:shadow-md transition-all group">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-sm ${index === 0 ? "bg-[#e8c84a] text-[#1a3d2b]" : index === 1 ? "bg-[#d4c9b0] text-[#1a3d2b]" : "bg-[#f5f0e8] text-[#8a9a8e]"}`}>
                              #{index + 1}
                            </div>
                            <div className="w-16 h-16 rounded-xl bg-[#f5f0e8] flex items-center justify-center relative overflow-hidden">
                              {item.img ? <Image src={item.img} alt={item.name} fill className="object-cover group-hover:scale-105 transition-transform duration-300" /> : <Package size={24} className="text-[#8a9a8e]" />}
                              {item.isOrganic && <div className="absolute top-0 right-0 bg-[#1a3d2b] text-[#e8c84a] text-[6px] font-black px-1 py-0.5 rounded-bl-lg">ORG</div>}
                            </div>
                            <div className="flex-1">
                              <h4 className="font-bold text-[#1a3d2b] group-hover:text-[#e86c2a] transition-colors">{item.name}</h4>
                              <p className="text-[10px] text-[#8a9a8e]">by {item.farmerId?.fullName || "Local Farmer"}</p>
                              <div className="flex items-center gap-2 mt-1">
                                ...
                                {item.isOrganic && <><span className="text-[9px] text-[#8a9a8e]">•</span><Leaf size={9} className="text-[#1a3d2b]" /></>}
                              </div>
                            </div>
                            <div className="text-right">
                              <span className="text-lg font-black text-[#e86c2a]">₹{item.price}</span>
                              <p className="text-[9px] text-[#8a9a8e]">per {item.unit}</p>
                            </div>
                            <ChevronRight size={16} className="text-[#8a9a8e] group-hover:text-[#1a3d2b] transition-colors" />
                          </Link>
                        ))}
                      </div>
                    ) : (
                      <div className="bg-white border border-[#d4c9b0] rounded-2xl p-8 text-center">
                        <Package size={40} className="mx-auto text-[#8a9a8e] mb-3" />
                        <p className="text-sm text-[#1a3d2b] font-semibold mb-1">No products yet</p>
                        <p className="text-xs text-[#8a9a8e]">Check back soon for fresh produce!</p>
                      </div>
                    )}
                  </div>
                </div>
