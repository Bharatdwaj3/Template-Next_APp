...
import {
  ...
  Users, Edit, Trash2, TrendingUp, Sparkles, Clock, Package as PackageIcon,
  Award, ChevronRight
} from 'lucide-react';
...
export default function FarmerProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return <Loader2 className="animate-spin text-[#1a3d2b]" size={40} />;
  if (!farmer) return (
    <div className="min-h-screen flex flex-col items-center justify-center">
      <p className="text-[#1a3d2b] mb-4">Farmer not found.</p>
      <Link href="/features/farmer" className="flex items-center gap-2 text-sm font-bold text-[#e86c2a] hover:underline">
  ...
  return (
    <ProtectedRoute>
      <div className="bg-[#1a3d2b] py-16 px-6 relative overflow-hidden">
        <span className="text-[8rem] font-black text-white/10 uppercase select-none whitespace-nowrap absolute right-4 top-1/2 -translate-y-1/2" style={{ writingMode: 'vertical-rl' }}>
          {userName}
        </span>
        <Link href="/features/farmer" className="absolute top-4 left-6 flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-white/60 hover:text-white transition-colors">
          <ArrowLeft size={13} /> All Farmers
        </Link>
        {isOwner && (
          <div className="absolute top-4 right-6">
            <button onClick={handleLogout} disabled={loggingOut} className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-white/60 hover:text-white border border-white/20 hover:border-white/40 px-4 py-1.5 rounded-xl transition-colors disabled:opacity-50">
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        ...
        <div className="grid grid-cols-3 gap-4 mb-10">
          ...
          <div className="bg-white border border-[#d4c9b0] rounded-2xl px-6 py-5 text-center">
            <p className="text-3xl font-black text-[#1a3d2b]">{followingCount}</p>
        ...
        <div className="flex gap-1 mb-8 bg-white rounded-xl p-1.5 border border-[#d4c9b0] w-fit">
          {['Produce', 'Sellers', 'Buyers', 'Location'].map((label, i) => (
            <button key={label} onClick={() => setActiveTab(i)} className={`px-6 py-2.5 text-sm font-bold uppercase tracking-wider rounded-lg transition-all ${activeTab === i ? 'bg-[#1a3d2b] text-[#e8c84a] shadow-sm' : 'text-[#4a5a4e] hover:text-[#1a3d2b] hover:bg-[#f5f0e8]'}`}>
              {label}
            </button>
          ))}
        </div>
        ...
        <div className="min-h-96">
          {activeTab === 0 && (
            <div>
              <div className="flex items-center justify-between mb-6">
                ...
                {isOwner && (
                  <button onClick={() => setShowCreateModal(true)} className="flex items-center gap-2 bg-[#1a3d2b] text-[#e8c84a] px-5 py-2.5 rounded-xl hover:bg-[#2a5a3b] transition-colors font-bold text-sm">
              ...
              {produceloading ? (
                  ...
                  {produceList.map((item) => (
                    <div key={item._id} className="bg-white border border-[#d4c9b0] rounded-xl p-4 hover:border-[#1a3d2b]/40 hover:shadow-md transition-all group">
                      ...
                      <div className="flex items-center justify-between">
                        <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-full ${item.stock > 10 ? 'bg-[#1a3d2b]/10 text-[#1a3d2b]' : 'bg-[#e86c2a]/10 text-[#e86c2a]'}`}>
                ...
                <div className="bg-white border border-[#d4c9b0] rounded-2xl p-12 text-center">
                  ...
                  <p className="text-lg text-[#1a3d2b] font-semibold mb-2">{isOwner ? 'No produce yet' : 'No produce listed'}</p>
                  <p className="text-sm text-[#8a9a8e] mb-6">{isOwner ? 'Create your first product listing' : "This farmer hasn't listed any produce yet"}</p>
                  {isOwner && (
                    <button onClick={() => setShowCreateModal(true)} className="flex items-center gap-2 bg-[#1a3d2b] text-[#e8c84a] px-5 py-2.5 rounded-xl hover:bg-[#2a5a3b] transition-colors font-bold text-sm mx-auto">
          ...
          {activeTab === 2 && (
            <div>
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-black text-[#1a3d2b] uppercase tracking-tight">Recently Viewed</h2>
                {visitedCount > 0 && (
                  <button onClick={clearHistory} className="text-[10px] font-bold text-[#e86c2a] hover:underline">Clear History</button>
                )}
              </div>
              {visitedLoading ? (
                <div className="flex items-center justify-center py-12">
                    ...
                    <Link key={item._id} href={`/features/produce/${item._id}`} className="flex items-center gap-4 p-4 bg-[#f5f0e8] rounded-xl hover:bg-[#e8e0d0] transition-all group">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          {item.isOrganic && (
                            <span className="text-[8px] bg-[#1a3d2b] text-[#e8c84a] px-1.5 py-0.5 rounded-full">ORGANIC</span>
                          )}
                        </div>
                ...
                {farmer.location?.coordinates && farmer.location.coordinates.length === 2 ? (
                  <>
                    ...
                    <div className="mt-4 p-4 bg-white rounded-lg border border-[#d4c9b0]">
                      ...
                      {isOwner && (
                        <Link href={`/features/farmer/${farmer._id}/edit-location`} className="inline-block mt-3 text-[10px] font-bold text-[#e86c2a] hover:underline">
                          Update Location →
                        </Link>
                      )}
                    </div>
                  ...
                  <div className="bg-white border border-[#d4c9b0] rounded-xl p-8 text-center">
                    ...
                    <p className="text-[#8a9a8e] mb-2">No location added yet</p>
                    {isOwner && (
                      <Link href={`/features/farmer/${farmer._id}/edit-location`} className="inline-block mt-2 text-sm font-bold text-[#e86c2a] hover:underline">
                        Add Farm Location →
                      </Link>
                    )}
                  </div>
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <form ref={formRef} className="p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-4">
                <button type="button" onClick={() => setShowCreateModal(false)} className="flex-1 px-4 py-2.5 border border-[#d4c9b0] rounded-xl hover:bg-[#f5f0e8] transition-colors font-bold text-sm text-[#4a5a4e]">Cancel</button>
                <button type="button" onClick={handleCreateProduce} className="flex-1 px-4 py-2.5 bg-[#1a3d2b] text-[#e8c84a] rounded-xl hover:bg-[#2a5a3b] transition-colors font-bold text-sm">Create Listing</button>
              </div>
            </form>
          </div>
        </div>
