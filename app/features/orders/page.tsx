  ...
  return (
    <div className="min-h-screen bg-[#f5f0e8] pt-24 pb-16 px-6">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-4xl font-black text-[#1a3d2b] mb-8">My Orders</h1>

        {loading ? (
          <div className="flex justify-center py-12">
            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#1a3d2b]" />
          </div>
        ) : orders.length === 0 ? (
          <div className="bg-white rounded-xl p-12 text-center">
            <Package size={48} className="mx-auto text-[#8a9a8e] mb-4" />
            <h2 className="text-xl font-black text-[#1a3d2b] mb-2">No Orders Yet</h2>
            <p className="text-[#8a9a8e] mb-6">Start shopping to see your orders here</p>
            <Link
              href="/features/produce"
              className="inline-block bg-[#1a3d2b] text-[#e8c84a] px-6 py-3 rounded-xl font-bold"
            >
              Browse Produce
            </Link>
          </div>
          ...
          <div className="space-y-4">
            {orders.map((order) => (
              <div key={order._id} className="bg-white rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="flex justify-between items-center pt-4 border-t border-[#d4c9b0]">
                  <div>
                  ...
                  <Link
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-[#f5f0e8] rounded-xl text-sm font-bold hover:bg-[#e8e0d0] transition-colors"
                  >
