...
export default function EditProfileModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res  = await api.put(`/${role}/${profileId}`, formData);
      onSuccess(res[role] ?? res);
      onClose();
    } catch (err: any) {
      ...
      <div className="relative z-10 w-full max-w-lg bg-bg rounded-2xl shadow-2xl border border-border overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="bg-primary px-6 py-4 flex items-center justify-between shrink-0">
          <div>
            ...
            <p className="text-base font-black text-text-inverse uppercase">Your Profile</p>
          </div>
          <button onClick={onClose} className="p-1.5 hover:bg-white/10 rounded-lg transition-colors">
            <X size={16} className="text-text-inverse/60" />
          </button>
        </div>
        ...
        <div className="overflow-y-auto p-6 space-y-5 flex-1">
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 rounded-xl overflow-hidden border border-border bg-primary/10 flex items-center justify-center shrink-0">
                {preview
              ...
              <button
                ...
                className="text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-xl border border-border text-primary hover:border-primary/40 transition-colors"
              >
                Choose Image
              </button>
              <input
          ...
          {role === 'farmer' && (
            <div>
              ...
              <input
                ...
                className="w-full bg-bg-alt border border-border rounded-xl px-4 py-2.5 text-sm text-primary focus:outline-none focus:border-primary/40"
              />
            </div>
          )}
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              maxLength={1000}
              className="w-full bg-bg-alt border border-border rounded-xl px-4 py-2.5 text-sm text-primary focus:outline-none focus:border-primary/40 resize-none"
            />
          ...
          {(role === 'farmer' || role === 'grocer') && (
            <div>
              ...
              <div className="flex flex-wrap gap-2 mb-2">
                {interests.map((s, i) => (
                  <span key={i} className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full bg-primary/10 text-primary border border-primary/15">
                    {s}
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="Add one..."
                  className="flex-1 bg-bg-alt border border-border rounded-xl px-4 py-2 text-sm text-primary focus:outline-none focus:border-primary/40"
                />
                <button
                  onClick={addInterest}
                  className="px-3 py-2 rounded-xl bg-primary text-accent hover:bg-primary/90 transition-colors"
                >
                  <Plus size={14} />
                </button>
              </div>
                  ...
                  <div key={i} className="flex gap-2 items-start">
                    <div className="flex-1 space-y-1.5">
                      <input
                        ...
                        className="w-full bg-bg-alt border border-border rounded-xl px-3 py-2 text-sm text-primary focus:outline-none focus:border-primary/40"
                      />
                      <input
                        value={addr.address}
                    ...
                    <button
                      ...
                      className="p-2 mt-1 hover:bg-cta/10 rounded-lg transition-colors"
                    >
                      <Trash2 size={14} className="text-cta" />
                    </button>
                  </div>
                ))}
                <button
                  onClick={addAddress}
                  className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-primary border border-dashed border-border px-4 py-2 rounded-xl hover:border-primary/40 transition-colors w-full justify-center"
                >
          ...
          {error && (
            <p className="text-[11px] font-black text-cta uppercase tracking-widest">{error}</p>
          )}
        </div>
        ...
        <div className="px-6 py-4 border-t border-border flex gap-3 shrink-0">
          <button
            onClick={onClose}
            className="flex-1 text-[10px] font-black uppercase tracking-widest py-3 rounded-xl border border-border text-primary hover:border-primary/40 transition-colors"
          >
            Cancel
          </button>
          <button
            ...
            disabled={saving}
            className="flex-1 flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest py-3 rounded-xl bg-primary text-accent hover:bg-primary/90 transition-colors disabled:opacity-50"
          >
