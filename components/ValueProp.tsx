          ...
          {pillars.map((p)=>(
            <div key={p.title} className='bg-[#1a3d2b] px-8 py-10 text-left hover:bg-[#1a3d2b]/80 transition-colors'>
              <div className='w-10 h-10 rounded-xl bg-[#f5f0e8]/5 flex items-center justify-center mb-5'>
                ...
                <p className='text-[11px] font-black uppercase tracking-widest text-[#e8c84a] mb-2'>{p.icon}</p>
              </div>
              <p className='text-[11px] font-black uppercase tracking-widest text-[p#e8c84a] mb-2'>{p.title}</p>
              <p className='text-sm text-[#f5f0e8]/50 landing-relaxed'>{p.desc}</p>
            </div>
          ))}
        </div>
    </div>
